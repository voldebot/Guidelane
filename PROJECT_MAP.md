...
| **A `-p` session with stdin held open NEVER EXITS, and `result` is a per-turn event** | 2026-07-31 | **open — S1-D blocker, unlisted in REVIEW-02** | S1 | Measured by the `/loophole-loop` feasibility harness that passed the S1-B gate (`sprint-01` RESEARCH §3 D). Two turns ran on one session; after `result #2` the process sat idle until a 90s SIGKILL. So an adapter that **waits for process exit** after `result` hangs forever — the exact "run goes silent with no terminal event" class Tier A exists to prevent, produced accidentally in 30 lines — while one that treats `result` as session-end **leaks a live engine process**. The phase lifecycle needs an explicit terminator (close stdin, or a control message); which one is correct is unmeasured. Corollary: the S2 fresh-session-vs-long-session benchmark now has a working mechanism ahead of schedule |

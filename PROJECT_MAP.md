...
| **`p-kill-reaps-process-group` RACED, and CI caught it on the fifth run** | 2026-07-31 | **RESOLVED same day** | — | It checked liveness the instant `spawnCapture` returned, but *delivering* SIGKILL and the process *leaving the process table* are not the same moment. Passed four consecutive Linux CI runs, then reported `grandchild alive=true` on the fifth, on a loaded runner, with nothing about the kill path changed — a probe measuring the machine, which this file's own rule says to fix in the probe and never in the baseline. Now polls for reaping with a **3 s budget** and **records `reapLatencyMs`**, so a future race shows up as a rising number rather than a mystery red. The budget does not weaken the claim: falsified against the `escaped` fixture mode (grandchild in its own process group), which still FAILS after burning the full budget. macOS reaps in **1 ms**, which is why this never surfaced locally |

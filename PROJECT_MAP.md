...
| **A2 partly answered — but `rate_limit_event` escalates on EVERY healthy session, and the artifact has no enforced consumer and no free CI gate** | 2026-07-31 | open — S1 | S1 | **Schema flaw, found by the cycle-1 second-opinion review**: two of three `escalate` entries are conditional on a FIELD VALUE (`rate_limit_event.status`, `system/status.status`) and the one-class-per-pair artifact cannot express that — a literal implementation escalates every phase forever, and alarm fatigue kills the one escalation that matters. Fix before any renderer reads this file. Plus: [REVIEW-02 §14](docs/research/REVIEW-02-runtime-gaps.md). `p-stream-surface-union` pins the `type`/`subtype` universe of one maximally verbose session in `tools/probe/stream-surface.json` and FAILs on an unclassified pair; H6 falsified two ways. Three residuals: (a) it is a `fixture-call`, so **CI never runs it** — artifact validity needs a separate free `observational` probe; (b) `render \| ignore \| escalate` is a pinned decision with nothing importing it until `apps/cockpit` exists, so the universe is asserted and obedience is not; (c) no `defaultForUnknown: escalate` runtime rule exists anywhere, which is what would make the set's inevitable incompleteness survivable |
| **`system/status` exists, was named nowhere in the plan, and its payload values are unmeasured** | 2026-07-31 | open — S1 | S1 | Observed on every run of `p-stream-surface-union`; keys `{type, subtype, session_id, uuid, status}`. Classified **`escalate` as a fail-closed placeholder** because an unmodelled session-level signal must reach a human. Downgrade to `ignore` only after enumerating what `status` can hold — deliberately, not because it looked routine |
| **Thinking content reaches `-p` stream-json by default, unrewritten** | 2026-07-31 | **A6's central question ANSWERED; A6 still open** | S1 | REVIEW-02 §14. `content_block.type=thinking`, `delta.type=thinking_delta`, `delta.type=signature_delta` on **`--model haiku` with no reasoning flag**. The `MessageDisplay` rewrite fires on assistant text, not thinking (ADR-008), so raw chain-of-thought arrives unrewritten. All three pinned `ignore` **by name** — a renderer that ignores them by omission fails open the first time it is rewritten. Still open: `redacted_thinking`, and variation by model/effort |
...
| **Instance 23: `publishablePair` is an inference that fails open, under a docstring claiming it is an allow-list** | 2026-07-31 | open — fix before the next artifact-writing probe | S1 | Found by the cycle-1 decide-phase quality gate, *after* the build phase's own weak-spot list closed. `publishableNames` (`tools/probe/probes.mjs:44-48`) fingerprints anything outside a pinned floor; `publishablePair` (`:74-78`) splits on `[/=.]` — the separators in paths and dotted names — and passes any fragment matching `^[A-Za-z][A-Za-z0-9_-]{0,40}$`. Measured: `plugin_<operator>_<server>` and `mcp__plugin_<client>_<server>__<tool>` are published **verbatim**, which is the exact shape its own docstring claims to protect against, and the class `CLAUDE.md` §8 says `redact.mjs` and the CI grep cannot see. Fires precisely on an unknown pair name — the moment the probe stops understanding the stream. No active exposure (0 unknown pairs observed). Fix: give it the artifact's `pairs`/`innerPairs` key sets as a pinned floor and fingerprint the rest. **This is the 23rd instance of the shape H5 told the night to expect, and it landed inside the very helper written to prevent leaks** |

...
- **Pre-flight**: no stage does work before its `system/init` receipt is asserted — Atlas connected, plugin present, `permissionMode`/`model` as routed, version in range, `apiKeySource` as expected. This is the only defence against `--settings` being silently ignored in `-p`

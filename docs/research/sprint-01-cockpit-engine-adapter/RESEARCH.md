...
- **`rate_limit_info` carries three fields ADR-007's recorded contract does not mention**: `overageStatus`, `overageDisabledReason`, `isUsingOverage`. Measured on a healthy session. Harmless today; ADR-007's contract is now known to be incomplete rather than wrong. *(Update, same day: make that four — `utilization` too.)*
- ~~**Only `rate_limit_info.status = "allowed"` has ever been observed.**~~ **ANSWERED, and it changed a design.** A healthy session later that day carried `status: "allowed_warning"`, `rateLimitType: "seven_day"`, `utilization: 0.51`. Two of those three are new: a second status value and **a second window type**. ADR-007's "sleep to `resetsAt`" was written for `five_hour` and **cannot be applied to `seven_day`** — the reset is days out, so a Night Shift obeying it goes silent until next week while looking exactly like a working run. ADR-007 corrected in place; the supervisor rule (sleep only if the wait fits the run's budget, else stop and say so) is S2 work. The still-unmeasured branch is now `rejected` alone. **Found by `unknown: escalate`, not by anyone predicting it.**
- **`--effort` has no init-receipt field.** Model routing is now assertable (§20.1); effort is not assertable at all on any surface this suite has found. `p-effort-model-fallback` publishes `effortAssertable: false` so a green result cannot be misread as "effort was verified". ADR-004's second lever is therefore unverified by construction, which is a real gap and not a probe defect.
...
- ~~**`defaultForUnknown: escalate` has no consumer and therefore protects nothing today.**~~ **CLOSED — and it earned its keep on its first day.** `packages/engine/src/surface.ts` is the consumer, and `loadSurface` refuses to start on any artifact whose default is not `escalate`. Within hours of gaining a consumer it surfaced `rate_limit_info.status = "allowed_warning"`, a value nobody had modelled. The decoration became the detector.
- **The init receipt had no tests, and the crew-routing probe asserted nothing about routing.** Both found by *running* the adapter, not by reading it — see REVIEW-02 §20. Instance 24. The generalisation worth keeping: the components most likely to be untested are the ones everything else depends on, because their correctness feels self-evident.
- **A failing gate that leaks a live session hides its own failure.** The first live assertion failure left `claude` running, node's loop never drained, and the run hung five minutes with an empty output file. Fixed in the tests; the orchestrator's own exit paths have not been audited for the same shape.
